...
#include<conio.h>
main(){
    int loan_amt,interest1,interest3,emi,i,j,k,l,n,z,temp,time4emi,foreclosure;
    ...
    printf("Enter time for Simple Interest(in years):");
    scanf("%d", &z);

    int SI=(loan_amt*interest1*z)/100;
    ...
    printf("Enter No. Of Installments(no. of months):");
    scanf("%d",&emi);
...
/*   for(i=0;i<emi;i++){
    ...
    if(n>foreclosure){
        ...
        int leftout= emi-foreclosure;//rest months of emi
        //int rest_amt=monthly_emi*leftout;

        printf("Now for leftout months we will calculate interest using principle that we took in starting");

        int rst_amt=(loan_amt/emi)*leftout;

        //printf("So, this much amount is left from foreclosure: %d \nAnd this will be interested differently \n therefore enter the interest for leftout EMIs:", rst_amt);

        printf("So, we will calc. interest on this amount : %d \nAnd this will be interested differently \n therefore enter the interest for leftout EMIs:", rst_amt);


        scanf("%d", &interest3);

        int SI2=(rst_amt*interest3)/100;
        printf("Simple Interest on Rest EMI: %d \n",SI2);

        int Total_amt2= rst_amt+SI2;
        printf("Total Amount to be paid for Rest EMIs: %d \n", Total_amt2);
